...
int dfs_cnt(Node* root, map<Node*, int>& cnt)
{
    if (! root)
        return 0;

    int l = dfs_cnt(root->child[0], cnt);
    int r = dfs_cnt(root->child[1], cnt);
